...
void lineBez::draw()
{
   if(n>1)
   {
      ...
      cout << data[0].x << ' ' << data[0].y << ' ' << n << endl;
      for(double t = 0.02; t <= 1; t += 0.02)
      {
         Point a = {0, 0};
         for(int i = 0; i < n; i++)
         {
            a.x += pef[i]*pow(1-t, n-i-1)*pow(t, i)*data[i].x;
            a.y += pef[i]*pow(1-t, n-i-1)*pow(t, i)*data[i].y;
         }
         lineB c = lineB(b.x, b.y, a.x, a.y, color, 1);
         c.draw();
         b = a;
      }
      lineB c = lineB(b.x, b.y, data[n-1].x, data[n-1].y, color, 1);
      c.draw();
   }
}
...
void addLine(act acts[1000], int &countDo, int color)
{
   ...
   int x = mousex();
   int y = mousey();
   lineB a(mousex(), mousey(), mousex(), mousey(), color);
   while(abs(mousex()-630) <= 560 and abs(mousey()-450) <= 350 and mousebuttons())
   {
      p = 1-p;
      setactivepage(p);
...
void addStar(act acts[1000], int &countDo, int color)
{
   ...
   int x = mousex();
   int y = mousey();
   star a(mousex(), mousey(), 0, 5, 0, color);
   while(abs(mousex()-630) <= 560 and abs(mousey()-450) <= 350 and mousebuttons())
   {
      p = 1-p;
      setactivepage(p);
      ...
      a.setCenter(x+(mousex()-x)/2, y+(mousey()-y)/2);
      a.setRadius(fmin(abs(mousex()-x)/2, abs(mousey()-y)/2));
      for(int i = 0; i < countDo; i++)
      {
         switch(acts[i].num)
         {
