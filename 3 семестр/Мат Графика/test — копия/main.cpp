...
int main()
{
   ...
   while(1)
   {
      ...
      for(int i = 1; i < 4; i++)
      {
         ...
         circle(romb[0].x+cos(a)*abs(param.x*x0+param.y*y0), romb[0].y+sin(a)*abs(param.x*x0+param.y*y0), 7);
      }
      ...
      circle(romb[0].x+(param.x+15)*cos(fi), romb[0].y+(param.x+15)*sin(fi), 6);
      ...
      else
      {
         ...
         for(int i = 1; i < 5; i++)
         {
            if(abs(mousex()-(romb[0].x+romb[i].x*param.x)) < 8 and abs(mousey()-(romb[0].y+romb[i].y*param.y)) < 8 and mousebuttons())
            {
               topull = i;
               change_figure(romb, param, i, fi);
            }
         }
         if(abs(mousex()-(romb[0].x+(param.x+15)*cos(fi))) < 7 and abs(mousey()-(romb[0].y+(param.x+15)*sin(fi))) < 7 and mousebuttons())
         {
            topull = 5;
