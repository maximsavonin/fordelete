...
Curve::Curve(const Curve &a): Figure(a.x0, a.y0, a.color, a.thickness), data(new int*[0]), n(a.n), step(a.step)
{
   for(int i = 0; i < n; i++)
   {
      data[i] = new int[2];
...
void addCurve(Figure *acts[], int &countFigure, int color)
{
    int all[100][2];
    int brea = 0, p = 0;
    int count = 0;
...
void addPicture(Figure *acts[], int &countFigure)
{
   string text;
   int t = 0, p = 0;
   while(1)
   {
      ...
      {
         acts[countFigure] = new Picture(10, 70, text);
         countFigure++;
         break;
      }
      t++;
   ...
   while(1)
   {
      ...
      if(abs(mousex()-630) <= 560 and abs(mousey()-450) <= 350 and mousebuttons() and mode != 0)
      {
         switch(mode)
         {
            case 1:
            {
               addLine(acts, countFigure, COLOR(col[0], col[1], col[2]));
               break;
            }
            case 2:
            {
               addRect(acts, countFigure, COLOR(col[0], col[1], col[2]));
               break;
            }
            case 3:
            {
               addCircle(acts, countFigure, COLOR(col[0], col[1], col[2]));
               break;
            }
            case 4:
            {
                addCurve(acts, countFigure, COLOR(col[0], col[1], col[2]));
                break;
            }
         }
      }
      ...
      if (abs(mousey() - 35) <= 25 and mousebuttons())
      {
          ...
          if (abs(mousex() - 1065) <= 25 and countFigure > 0)
          {
