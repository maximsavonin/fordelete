...
void drawgame(long int t, int timer, int &play, int &tomenu) // отрисовка игры
{
   p = 1-p;
   setactivepage(p);
...
void shift(int x, int y, int s) // генерация лабиринта
{
   while(1)
   {
      ...
      int n = 0;
      for(int i = 0; i < 4; i++)
      {// выбираем ячейки в которых небыли или с шансом 1/300 ячейку в которой были
         if(i%2) if(i/2)
            {
...
void control(long int &t, int &play, int &tomenu) // управление игрой
{
   if(1 == kbhit())
   {
      int Key = getch();
...
void drawmenu(int dif, int but, IMAGE *picmenu) // отрисовка меню
{
   p = 1-p;
   setactivepage(p);
   clearviewport();
   putimage(0, 0, picmenu, COPY_PUT);
...
void controlmenu(int &dif, int &but, int &timer, int &play, int &exitt) // управление меню
{
   but = -1;
   for(int i = dif*3; i < (dif+1)*3+dif; i++)
   {
...
int main()
{
   ...
   IMAGE *picmenu;
   picmenu = loadBMP("menu.bmp");
   
   int dif = 0, but = 0; // выбираем сложность? и на какую кнопку навели
   int timer = 0, tomenu = 0; // время для прохождение и пауза
   ...
   while(!exitt)
   {
      if(play)
      {
         ...
         if(!tomenu) control(t, play, tomenu);
         drawgame(t, timer, play, tomenu);
      }
      else
      {
         t = time(NULL);
         controlmenu(dif, but, timer, play, exitt);
         drawmenu(dif, but, picmenu);
      }
   }
}