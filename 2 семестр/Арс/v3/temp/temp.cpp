...
void drawmenu()
{
   Button buttons[6] = {{width/2, 300, 120, 30, "Играть"}, {width/2, 400, 120, 30, "Об игре"}, {width/2, 500, 120, 30, "Выход"}, {width/2, 300, 120, 30, "Легкая"}, {width/2, 400, 120, 30, "Средняя"}, {width/2, 500, 120, 30, "Сложная"}};
   p = 1-p;
   setactivepage(p);
   clearviewport();
   for(int i = 0; i < 3; i++)
   {
      ...
      outtextxy(buttons[i].x, buttons[i].y, _abracadabra_cast(buttons[i]);