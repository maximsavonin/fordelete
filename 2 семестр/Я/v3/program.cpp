...
void down(int &falling) // падение квадратиков
{
   for(int i = n_x; i > 0; i--)
      {
...
void checking(int &kol) // проверка 3 в ряд
{
   int again = 1, max_n = 0;
   while(again == 1)
   {
...
void new_column(int p)// появлеие следующей колонны
{
   for(int i = 1; i < n_x+1; i++)
   {
      if(map[i][2] != 0)
...
int main()
{
   int del = 0, falling = 0; //задержка между кадрами и падают ли что-либо в предыдущем кадре
   int numbut = 0, p = 0; // номер кнопки в меню и номер страницы
   srand(time(0));
