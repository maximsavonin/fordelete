...
void drawmenu(){ // отрисовка меню
   ...
   outtextxy(220, 100, "Фантастические змеи");
   outtextxy(200, 140, "и деревянные листницы");
   
   setusercharsize(4, 5, 1, 1);
   for(int i = 0; i < 4; i++){
      rectangle(325, 250+i*100, 525, 300+i*100);
   }
   outtextxy(380, 260, "Играть");
   outtextxy(365, 360, "Об игре");
   outtextxy(365, 460, "Об авторе");
   outtextxy(375, 560, "Выход");
   
   setvisualpage(p);
}

void abovegame(){ // отрисовка об игре
   while(1){
      p = 1-p;
      setactivepage(p);
      clearviewport();
...
void above(){ // отрисовка об авторе
   while(1){
      p = 1-p;
      setactivepage(p);
      clearviewport();
      ...
      settextstyle(SIMPLEX_FONT, HORIZ_DIR, USER_CHAR_SIZE);
      setusercharsize(1, 1, 3, 2);
      outtextxy(350, 150, "Об авторе");
      
      setusercharsize(1, 2, 1, 2);
      outtextxy(50, 250, "Всем привет, я Малахов Владислав, студент ЮУРГУ группы");
      outtextxy(50, 275, "ЕТ-211.'Фантастические змеи и деревянные лестницы' моя первая");
      outtextxy(50, 305, "корректно работающая игра!");
      outtextxy(50, 330, "Наслаждайтесь игровым процессом!");
