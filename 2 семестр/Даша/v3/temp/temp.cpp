...
int w = 1300, h = 800, radius = 20, speed = 3; //размер экрана, шара и скорость иъ перемещени€ по спирали
...
{
   zu_ball[0] += 20*cos(zu_ball[3]);
   zu_ball[1] += 20*sin(zu_ball[3]);
   ...
   for(int i = 0; i < n; i++)
   {
      ...
      if(sqrt(x*x+y*y) <= radius+1)
      {
         ...
         for(int k = 0; k < n; k++)
         {
            setball(Balls[k], _abracadabra_cast(Balls[k]);