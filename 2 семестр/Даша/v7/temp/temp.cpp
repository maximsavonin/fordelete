...
void goBall(int a, int b) //движиение по троектории одного шара
{
   while(1)
   {
      double x, y, xv, yv;
...
void setBall(int a, double fi) // функция задания координат шара по углу
{
   Balls[a].fi = fi;
...
void createBall(int a) // создаёт новый шар в начале пути
{
   Balls[a].color = rand() % 6 + 1;
   setBall(a, 6.5*pi);
}

void newgame() // задание переменных для новой игры
{
   n = 1;
   score = 0;
   createBall(0);
...
void flyBall() // полёт шара после выстрела
{
   ...
   for(int i = 0; i < n; i++)
   {
      ...
      if(sqrt(x*x+y*y) <= radius+1)// попали в шар
      {
         ...
         for(int k = 1; k <= i; k++)
         {
            if(Balls[i-k].color == zu_ball.color) indentic += 1;
            
            if(Balls[i-k].color != zu_ball.color || k == i && Balls[i-k].color == zu_ball.color)
            {
               left = k-1;
               break;
            }
         ...
         for(int k = 1; k+i < n; k++)
         {
            ...
            if(Balls[i+k].color != zu_ball.color || k+i+1 == n && Balls[i+k].color == zu_ball.color)
            {
               if(k+i+2 == n && Balls[i+k].color == zu_ball.color) k++;
               right = k-1;
               break;
            }
         }
...
void print_menu(int button, IMAGE *pic) // отрисовка главного меню
{
   clearviewport();
...
int main()
{
   ...
   int l = 0; // количество шаров и таймер появления нового
   ...
   while(1)
   {
      ...
      else
      {
         ...
         for(int i = n-2; i >= 0; i--)// смещение остальных шаров
         {
            for(int k = i+1; k < n; k++)
            {
               goBall(i, k);
         ...
         l++;
         if(l == radius*2/speed && score < 2500) // добаление нового шара
         {
            createBall(n);
            l = 0;
            n++;
         ...
         fillpoly(4, po);
         colorball(zu_ball.color);
         fillellipse(zu_ball.x, _abracadabra_cast(zu_ball);