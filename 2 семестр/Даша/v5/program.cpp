...
void newgame(Ball Balls[150], double zu_ball[5], int &n) // задание переменных дл€ новой игры
{
   n = 1;
   score = 0;
   creatball(Balls[0]);
...
void flyb(Ball Balls[150], double zu_ball[5], int &n, int &score) // полЄт шара после выстрела
{
   ...
   for(int i = 0; i < n; i++)
   {
      ...
      if(sqrt(x*x+y*y) <= radius+1)// попали в шар
      {
         ...
         {
            copyball(Balls[k+indentic], Balls[k]);
         }
         score += 50*indentic;
         zuballC(Balls, zu_ball, n);
         return;
...
void printmenu(int but) // отрисовка главного меню
{
   clearviewport();
   settextstyle(DEFAULT_FONT, HORIZ_DIR, 8);
...
void managmenu(int &but) // обработка главного меню
{
   ...
   if(mousebuttons() == 1 && but == 1) // открывает вкладку об игре
   {
...
int main()
{
   ...
   while(1)
   {
      ...
      else
      {
         ...
         po[0] = w/2.0 + 60*cos(fim); // координаты углов треугольника
         ...
         while(1) // последний шар сдвигаетс€ как бы толка€ следующие
         {
            Balls[n-1].fi -= 0.001;
            double x, y;
